...
/* global autodescription, autodescriptionL10n, confirm */

/* Advanced Optimizations don't work well with the title character length */
...
window[ 'autodescription' ] = {
	...
	/**
	 * Status bar description output on hover leave.
	 *
	 * @since 2.6.0
	 *
	 * @function
	...
	dynamicPlaceholder: function( event ) {
		...
		if ( typeof $tagbox !== "undefined" && $tagbox.length > 0 && ! $tagbox.is( ':checked' ) ) {
			...
			if ( $after ) {
				var $additions = $separator + " " + autodescriptionL10n['titleAdditions'];
				...
				var $additions = autodescriptionL10n['titleAdditions'] + " " + $separator;
